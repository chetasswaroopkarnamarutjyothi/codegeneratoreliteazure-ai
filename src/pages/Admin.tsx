...
export default function Admin() {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      const { data: profileData } = await supabase
        .from("profiles").select("email").eq("user_id", session.user.id).single();
      if (profileData?.email === "kchetasswaroop@gmail.com") setIsSuperAdmin(true);
  ...
  const fetchAdminCredits = async (userId: string) => {
    ...
    } catch (error: any) { console.error("Failed to fetch admin credits:", error); }
  };
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (pointsData) {
        ...
        pointsData.forEach((p) => { pointsMap.set(p.user_id, p); });
  ...
  const handleTransferToBank = async () => {
    ...
    try {
      const { error } = await supabase.rpc("transfer_to_approval_bank", { amount });
      if (error) throw error;
      toast({ title: "Transfer successful!", description: `${amount.toLocaleString()} credits moved to Approval Bank` });
      setTransferAmount("");
  ...
  const handleGrantFromBank = async () => {
    ...
    if (isNaN(points) || points <= 0) {
      toast({ title: "Invalid points", variant: "destructive" }); return;
    }
    if (adminCredits && points > (adminCredits.approval_bank_credits || 0)) {
      toast({ title: "Insufficient bank credits", variant: "destructive" }); return;
    }
    try {
      const { error } = await supabase.rpc("grant_credits_from_bank", {
        target_user_id: selectedUser.user_id, amount: points, grant_reason: grantReason || null,
      });
      if (error) throw error;
      toast({ title: "Credits granted!", description: `${points.toLocaleString()} credits sent to ${selectedUser.full_name}` });
      setSelectedUser(null); setGrantPoints(""); setGrantReason("");
  ...
  const handleBlockUser = async (profile: UserProfile) => {
    try {
      ...
      const { error } = await supabase.from("profiles").update({ is_blocked: newBlockedState }).eq("user_id", profile.user_id);
      if (error) throw error;
      toast({ title: newBlockedState ? "User blocked" : "User unblocked" });
      await fetchUsers();
    } catch (error: any) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    }
  ...
  const handlePromoteToEmployee = async (profile: UserProfile) => {
    try {
      const { data: existingRole } = await supabase
        .from("user_roles").select("role").eq("user_id", profile.user_id).eq("role", "employee").maybeSingle();
      if (existingRole) { toast({ title: "Already an employee" }); return; }
      const { error: roleError } = await supabase.from("user_roles").insert({ user_id: profile.user_id, role: "employee" });
      if (roleError) throw roleError;
      await supabase.from("employee_ldap").insert({ user_id: profile.user_id }).throwOnError();
      await supabase.from("user_points").update({ daily_points: 100 }).eq("user_id", profile.user_id);
      toast({ title: "User promoted!", description: `${profile.full_name} is now an employee.` });
      await fetchUsers();
    } catch (error: any) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    }
  ...
  const filteredUsers = users.filter(
    (u) => u.email.toLowerCase().includes(searchQuery.toLowerCase()) || u.full_name.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        {adminCredits && (
          <div className="grid md:grid-cols-4 gap-4 mb-6">
            ...
            <Card className="glass">
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-accent/20"><Crown className="w-5 h-5 text-accent" /></div>
                  <div>
            ...
            <Card className="glass border-green-500/30">
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-green-500/20"><Banknote className="w-5 h-5 text-green-500" /></div>
                  <div>
                    <p className="text-sm text-muted-foreground">Approval Bank</p>
                    <p className="text-2xl font-bold text-green-500">{(adminCredits.approval_bank_credits || 0).toLocaleString()}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card className="glass">
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-yellow-500/20"><Landmark className="w-5 h-5 text-yellow-500" /></div>
                  <div>
        ...
        <Tabs defaultValue="requests" className="space-y-6">
          <TabsList className="flex flex-wrap gap-1 h-auto w-full max-w-7xl p-2">
            ...
            <TabsTrigger value="doc-gen" className="flex items-center gap-1">
              <FileText className="w-4 h-4" />
              <span className="hidden sm:inline">Doc Gen</span>
            </TabsTrigger>
            ...
            {selectedUser && (
              <Card className="glass glow-border border-green-500/30">
                <CardHeader>
                  ...
                  <CardDescription>
                    Grant credits from Approval Bank (Available: {(adminCredits?.approval_bank_credits || 0).toLocaleString()})
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label>Credits to Grant</Label>
                    <Input type="number" placeholder="Enter credits" value={grantPoints} onChange={(e) => setGrantPoints(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button onClick={handleGrantFromBank} className="flex-1 bg-green-600 hover:bg-green-700">
                      <Gift className="w-4 h-4 mr-2" /> Grant from Bank
                    </Button>
                    <Button variant="outline" onClick={() => { setSelectedUser(null); setGrantPoints(""); setGrantReason(""); }}>Cancel</Button>
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            <Card className="glass">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6 p-4 rounded-lg bg-muted/30">
                  <div className="text-center"><p className="text-2xl font-bold text-primary">{users.length}</p><p className="text-xs text-muted-foreground">Total Users</p></div>
                  <div className="text-center"><p className="text-2xl font-bold text-green-500">{Array.from(userPoints.values()).filter(p => p.is_premium).length}</p><p className="text-xs text-muted-foreground">Premium</p></div>
                  <div className="text-center"><p className="text-2xl font-bold text-destructive">{users.filter(u => u.is_blocked).length}</p><p className="text-xs text-muted-foreground">Blocked</p></div>
                  <div className="text-center"><p className="text-2xl font-bold text-accent">{Array.from(userPoints.values()).reduce((s, p) => s + (p.daily_points || 0), 0).toLocaleString()}</p><p className="text-xs text-muted-foreground">Total Credits</p></div>
                </div>
                <div className="space-y-3">
                  {filteredUsers.map((profile) => {
                    const pts = userPoints.get(profile.user_id);
                    const isUserAdmin = pts?.daily_points === 685000 || pts?.approval_bank_credits === 285000;
                    return (
                      <div key={profile.id} className={`p-4 rounded-lg border transition-all ${profile.is_blocked ? "bg-destructive/10 border-destructive/30" : isUserAdmin ? "bg-yellow-500/5 border-yellow-500/30" : "bg-muted/30 border-border/50 hover:border-primary/30"}`}>
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <p className="font-semibold">{profile.full_name || "No Name"}</p>
                              {profile.is_blocked && <Badge variant="destructive" className="text-xs"><Ban className="w-3 h-3 mr-1" />Blocked</Badge>}
                              {isUserAdmin && <Badge className="bg-yellow-500/20 text-yellow-500 text-xs"><Shield className="w-3 h-3 mr-1" />Admin</Badge>}
                              {pts?.is_premium && !isUserAdmin && <Badge className="bg-purple-500/20 text-purple-500 text-xs"><Crown className="w-3 h-3 mr-1" />Premium</Badge>}
                            </div>
                            <p className="text-sm text-muted-foreground">{profile.email}</p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 p-3 rounded bg-background/50">
                              <div><p className="text-xs text-muted-foreground">Daily</p><p className="font-bold text-primary">{(pts?.daily_points || 0).toLocaleString()}</p></div>
                              <div><p className="text-xs text-muted-foreground">Monthly</p><p className="font-bold">{(pts?.monthly_points || 0).toLocaleString()}</p></div>
                              {isUserAdmin && <div><p className="text-xs text-muted-foreground">Approval Bank</p><p className="font-bold text-green-500">{(pts?.approval_bank_credits || 0).toLocaleString()}</p></div>}
                              <div><p className="text-xs text-muted-foreground">Age</p><p className="font-medium">{profile.age || "N/A"}</p></div>
                            </div>
                          ...
                          <div className="flex flex-col gap-2 ml-4">
                            <Button variant="outline" size="sm" onClick={() => setSelectedUser(profile)} disabled={profile.is_blocked}>
                              <Gift className="w-4 h-4 mr-1" /> Grant
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => handlePromoteToEmployee(profile)} disabled={profile.is_blocked} className="border-amber-500/30 hover:bg-amber-500/10 text-amber-500">
                              <UserPlus className="w-4 h-4 mr-1" /> Employee
                            </Button>
                            <Button variant={profile.is_blocked ? "default" : "destructive"} size="sm" onClick={() => handleBlockUser(profile)}>
                              {profile.is_blocked ? <><CheckCircle className="w-4 h-4 mr-1" />Unblock</> : <><Ban className="w-4 h-4 mr-1" />Block</>}
                            </Button>
                          </div>
                        </div>
                      </div>
                  ...
                  {filteredUsers.length === 0 && (
                    <div className="text-center py-8 text-muted-foreground">
                      <Users className="w-12 h-12 mx-auto mb-4 opacity-50" /><p>No users found</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          <TabsContent value="transfer">
            <Card className="glass">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><ArrowRightLeft className="w-5 h-5 text-green-500" /> Transfer to Approval Bank</CardTitle>
                <CardDescription>Move credits from daily to Approval Bank for granting</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex gap-4 items-end">
                  ...
                  <Button onClick={handleTransferToBank} disabled={isTransferring || !transferAmount} className="bg-green-600 hover:bg-green-700">
